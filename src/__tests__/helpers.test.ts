...
describe('helpers fucntions', () => {
  ...
  test('reduceUniqueIds', () => {
    ...
    const ids = randomIds.reduce<number[]>((ids, id) => reduceUniqueIds(ids, id), []);
    const keys = randomObjs.reduce<string[]>((keys, { key }) => reduceUniqueIds(keys, key), []);
  ...
  test('sortByDate', () => {
    const randomObjs = [
      { id: 12, date: '5-15-2022' },
      { id: 12, date: '5-12-2022' },
      { id: 12, date: '5-1-2022' },
    ];
